...
上一节把 JEPA 钉在了"表示空间如何学习"这条轴上，它交给我们的是一个高质量的单帧 latent $$z_t$$。但一个好的单帧表示，仍然只是一次**观测**，而不是一个**状态**。这个区别在静态表征学习里无关紧要，可一旦真的沿时间轴往前推演，它就立刻变成硬约束——这正是另一条轴（表示如何随时间传播）逼我们面对的第一个问题。
...
为什么对象世界模型**必须**有记忆？第 7 节已经给了一半答案：单帧只是观测而非状态，速度、被遮挡的物体、任务阶段这些跨时间的量必须被存在某处。对象化让这个需求变得更尖锐——一个物体可能滚出画面几秒后再滚回来，它的身份、位置和运动必须在**完全没有观测**的那段时间里被持续持有；一次碰撞的后果也可能几十帧后才显现。也就是说，object-centric world model 最核心的几项能力（对象永久性、跨遮挡的身份追踪、长程因果）本质上都是**记忆任务**：没有一块能跨时间持有对象信念的存储，第 8 节的 belief state 就无处安放。

问题只剩下这块存储**长什么样**。而它显然不能是随便一块均质内存——这就回到了与第 4 节完全同构的困境。

### 9.1 同一个困境，换了模态

为 object-centric world model 设计记忆时，两种朴素方案都会失败：
...
（其中 $$m_t^{i,\text{interaction}}$$ 是来自其他对象的交互消息，见第 11 节。）为记号简洁，这里用单一 $$h_t^i$$ 指代该对象的完整 belief state，隐去了第 8 节 RSSM 里确定性 $$h$$ 与随机 $$z$$ 的拆分——后文第 13、14 节谈到 prior/posterior 时，$$h_t^i$$ 仍指这个整体信念状态。它保存那些跨越短窗口、必须持续追踪的内容：
...
到这里，记忆系统本身已经完整：每个对象都有自己的四层存储，也有一套把它们融合成增强状态 $$\tilde{h}_t^i$$ 的读取流程。但第 9、10 节藏着一个未言明的假设——每一步都是**对象各自进行**的：对象 $$i$$ 只读自己的历史与全局场景，它的状态更新里从头到尾没有出现过另一个对象 $$j$$。存储做得再精细，这仍然是一套"各自记各自"的机制。而真实世界的麻烦恰恰在于，一个对象的状态演化大量来自**别的对象**——这是记忆层解决不了、必须单独补上的一环。
